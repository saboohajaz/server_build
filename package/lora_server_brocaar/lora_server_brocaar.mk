...
LORA_SERVER_BROCAAR_APP_SERV_VER = fleet-tufwrap-0.20.1
...
LORA_SERVER_BROCAAR_AS_FLEET = $(LORA_SERVER_BROCAAR_FLEET_BITBUCKET)/$(LORA_SERVER_BROCAAR_A)
...
LORA_SERVER_BROCAAR_SAS_DIR = src/$(LORA_SERVER_BROCAAR_GITHUB)
LORA_SERVER_BROCAAR_SAS_FLEET = src/$(LORA_SERVER_BROCAAR_AS_FLEET)
...
define LORA_SERVER_BROCAAR_BUILD_CMDS
	...
	@GOPATH=$(@D) $(HOST_GO_FLEET_ROOT)/bin/go get -u $(LORA_SERVER_BROCAAR_AS_FLEET) || true
	cd $(@D)/$(LORA_SERVER_BROCAAR_SAS_FLEET) && git checkout tags/$(LORA_SERVER_BROCAAR_APP_SERV_VER)
	mkdir -p $(@D)/$(LORA_SERVER_BROCAAR_SAS_DIR)
	mv $(@D)/$(LORA_SERVER_BROCAAR_SAS_FLEET) $(@D)/$(LORA_SERVER_BROCAAR_SAS_DIR)
